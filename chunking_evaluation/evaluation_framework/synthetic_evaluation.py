    ...
    def _extract_question_and_references(self, corpus, document_length=4000, prev_questions=[]):
        ...
        completion = self.client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": self.question_maker_user_prompt.replace("{document}", document).replace(
                        "{prev_questions_str}", prev_questions_str
                    ),
                },
            ],
        ...
        while i < n:
            while True:
                try:
                    ...
                    new_question = {"question": question, "references": json.dumps(references), "corpus_id": corpus_id}
